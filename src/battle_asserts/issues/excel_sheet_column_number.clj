...
(def description
  {:en "Given a column title (as it appears in an Excel sheet), return its corresponding column number.
        For example: A -> 1, B -> 2, C -> 3, ...  Z -> 26, AA -> 27, AB -> 28"
   :ru "Дано имя столбца (как оно отображается на странице Excel), верните соответствующий номер этого столбца.
        Например: A -> 1, B -> 2, C -> 3, ...  Z -> 26, AA -> 27, AB -> 28"})
