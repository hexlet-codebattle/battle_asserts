...
(def description "Create a function that applies a `discount` to every `price` of the product in the array. Use floor rounding.")
...
(def test-data
  [{:expected [1 2 3 5] :arguments [[2 4 6 11] 50]}
   {:expected [8 17 35 89] :arguments [[10 20 40 100] 10]}
   {:expected [1 2 4 10] :arguments [[10 20 40 100] 90]}])

(defn apply-discount [price discount]
  (int (Math/floor (- price (* price (float (/ discount 100)))))))
